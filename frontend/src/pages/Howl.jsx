...
function Howl() {
  ...
  const changeHowl = async (e) => {
    ...
    try {
      ...
      const res = await fetch(
        `https://howler-backend.onrender.com/api/howl/changeHowl/${howlId}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
  ...
  const deleteHowl = async () => {
    try {
      await fetch(
        `https://howler-backend.onrender.com/api/howl/silenceHowl/${howlId}`,
        {
          method: "DELETE",
        }
      );
  ...
  if (isAuthenticated) {
      ...
      <div className="container">
        <div className="ps-4 mt-4">
          ...
          {user && user.nickname == howl.howler && (
            <div>
              <form onSubmit={changeHowl} encType="multipart/form-data">
                <input
                  type="text"
                  name="caption"
    ...
    return (
      <div className="container">
        <div className="ps-4 mt-4">
          <p>{howl.caption}</p>
          <p className="fw-bold">{howl.howler}</p>
          <p>{howl.updated_at}</p>
        </div>
      </div>
    );
